...
function refreshMainButtonsToolTipPlacement() {
    if (!isMobileDevice) {
        ...
        setTippy('endCallForEveryoneButton', 'End call for everyone', bPlacement);
...
function roomIsReady() {
    ...
    BUTTONS.main.endCallForEveryoneButton && isPresenter ? show(endCallForEveryoneButton) : hide(endCallForEveryoneButton);
    ...
    BUTTONS.settings.videoSharing && show(tabVideoShareBtn) && show(videoShareButton);
...
function handleButtons() {
    ...
    endCallForEveryoneButton.onclick = () => {
        endCallForEveryone();
    };
    shareButton.onclick = () => {
        shareRoom(true);
    ...
    searchParticipantsFromList.oninput = () => {
        rc.searchPeer();
    };
...
function handleEditor() {
    ...
    if (rc.editorIsLocked() && !isPresenter) {
        quill.enable(false);
    }
...
async function endCallForEveryone() {
    if (!isPresenter) {
        return userLog('warning', 'Only admins can end the webinar for everyone', 'top-end', 5000);
    }
    ...
    const result = await Swal.fire({
        ...
        text: 'This will disconnect all attendees and end the webinar immediately.',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonText: 'End for everyone',
...
function whiteboardAction(data, emit = true) {
    if (emit) {
        ...
        // Skip noisy join-time status toasts for lock/unlock sync actions.
        if (data.action === 'lock' || data.action === 'unlock') {
            // Apply the action silently.
        } else {
            userLog(
                'info',
...
async function getRoomParticipants() {
    ...
    if (rc?.isToggleRaiseHand) rc.applyRaiseHandsFilter();
    if (rc?.isToggleUnreadMsg) rc.applyUnreadMsgFilter();
...
function getParticipantsList(peers) {
    ...
    for (const peer of Array.from(peers.keys())) {
        ...
        const peer_is_admin = peer_info.peer_presenter;
        const peer_admin_badge = peer_is_admin
            ? `<i class="fa-solid fa-user-shield" style="color:gold;margin-left:4px" title="Admin"></i>`
            : '';
        ...
        if (socket.id !== peer_id) {
            ...
            if (isRulesActive && isPresenter) {
                ...
                >
                    ...
                    />
                    <div class="about">
                        <div class="name">${peer_name_limited}${peer_admin_badge}</div>
                ...
                if (BUTTONS.participantsList.grantPresenterButton) {
                    if (peer_info.peer_presenter) {
                        li += `<li><button class="btn-sm ml5" id='${peer_id}___revokePresenter' onclick="rc.grantPresenter('${peer_id}','${peer_name}','${peer_info.peer_uuid}',false)"><i class="fa-solid fa-user-shield" style="color:red"></i> Revoke admin</button></li>`;
                    } else {
                        li += `<li><button class="btn-sm ml5" id='${peer_id}___grantPresenter' onclick="rc.grantPresenter('${peer_id}','${peer_name}','${peer_info.peer_uuid}',true)"><i class="fa-solid fa-user-shield" style="color:gold"></i> Make admin</button></li>`;
                    }
                }
...
function showAbout() {
    ...
    Swal.fire({
        ...
        title: BRAND.about?.title && BRAND.about.title.trim() !== '' ? BRAND.about.title : 'WebRTC SFU v2.1.19',
        html: `
            <br />
            <div id="about">
