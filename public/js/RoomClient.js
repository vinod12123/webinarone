...
class RoomClient {
    constructor(
        ...
        this.activeSpeechElemId = null;
        this.activeSpeechButtonId = null;
    ...
    async join(data) {
        this.socket
            ...
            .then(async (room) => {
                ...
                if (room === 'nameAlreadyInRoom') {
                    console.warn('00-WARNING ----> Username already active in room');
                    return this.userNameAlreadyInRoom();
                }

                if (room === 'alreadyJoined') {
                    console.warn('00-WARNING ----> You are already connected to this room');
                    return this.userAlreadyConnected();
                }
    ...
    async handleRoomInfo(room) {
        ...
        for (let peer of Array.from(this.peers.keys()).filter((id) => id == this.peer_id)) {
            ...
            isPresenter = window.sessionStorage.isReconnected === 'true' ? isPresenter : my_peer_info.peer_presenter;
            this.peer_info.peer_presenter = isPresenter;
            this.getId('isUserPresenter').innerText = isPresenter;
            window.sessionStorage.isReconnected = false;
            ...
            this.isEditorLocked = room.config.isEditorLocked;

            handleRules(isPresenter);
            this.syncEditorAccessUI();
    ...
    initSockets() {
        ...
        this.socket.on('presenterUpdated', this.handlePresenterUpdated);
    }
    ...
    // ####################################################
    // HANDLE PRESENTER (ADMIN) ACCESS GRANTED / REVOKED
    // ####################################################

    handlePresenterUpdated = (data) => {
        const { granted, from_peer_name, to_peer_name } = data;

        console.log('[presenterUpdated] Received', data);

        // Update the global isPresenter flag and peer_info
        isPresenter = !!granted;
        this.peer_info.peer_presenter = isPresenter;
        this.getId('isUserPresenter').innerText = isPresenter;

        // Re-apply UI rules based on the new role.
        // Pass true so the "auto-detected" roomAction emissions (hostOnlyRecording,
        // lobby, broadcasting, moderator broadcast) are skipped — they must not
        // fire again mid-session, only on initial room join.
        handleRules(isPresenter, true);

        // Whiteboard UI may still be in attendee-locked state if this peer is promoted
        // while a whiteboard lock was active. Restore full whiteboard controls for admins.
        if (granted) {
            elemDisplay('whiteboardTitle', true, 'flex');
            elemDisplay('whiteboardOptions', true, 'flex');
            elemDisplay('whiteboardButton', true);
            if (typeof wbDrawing === 'function') wbDrawing(true);
        } else if (typeof wbIsLock !== 'undefined' && wbIsLock) {
            elemDisplay('whiteboardTitle', false);
            elemDisplay('whiteboardOptions', false);
            elemDisplay('whiteboardButton', false);
            if (typeof wbDrawing === 'function') wbDrawing(false);
        }

        // Keep editor access/controls in sync after runtime role changes.
        this.syncEditorAccessUI();

        // Sync Moderator UI switches if now a presenter
        this.syncModeratorUI();

        // Refresh the participants list so the caller's own icon updates
        getRoomParticipants();

        const action = granted ? 'granted' : 'revoked';
        const icon = granted ? '⭐️' : '🚫';
        const msg = `${icon} Admin access <strong>${action}</strong> by <strong>${from_peer_name}</strong>`;
        this.userLog('info', msg, 'top-end', 8000);
        this.sound(granted ? 'notify' : 'ping');
    };

    /**
     * Grant or revoke presenter (admin) access for a remote peer.
     * Only callable by the current presenter.
     *
     * @param {string} to_peer_id   - Socket ID of the target peer
     * @param {string} to_peer_name - Display name of the target peer
     * @param {string} to_peer_uuid - UUID of the target peer
     * @param {boolean} grant       - true = grant admin, false = revoke admin
     */
    grantPresenter(to_peer_id, to_peer_name, to_peer_uuid, grant) {
        if (!isPresenter) {
            return this.userLog('warning', 'Only a presenter can share admin access', 'top-end', 4000);
        }
        Swal.fire({
            background: swalBackground,
            position: 'center',
            imageUrl: grant ? image.users : image.user,
            title: grant ? `Make <strong>${to_peer_name}</strong> an admin?` : `Revoke admin from <strong>${to_peer_name}</strong>?`,
            html: grant
                ? 'This participant will gain full presenter capabilities (mute, kick, lock room, etc.)'
                : 'This participant will return to guest role.',
            showDenyButton: true,
            confirmButtonText: grant ? 'Yes, make admin' : 'Yes, revoke',
            ...
            this.socket.emit('grantPresenter', data);
            // Refresh participants list locally so the button toggles immediately
            getRoomParticipants();
        });
    }
    ...
    handleDisconnect(reason) {
        ...
        window.sessionStorage.isReconnected = true;
    ...
    userNameAlreadyInRoom() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                endRoomSession();
                openURL((window.location.href = '/login'));
            }
        });
    }
    ...
    userAlreadyConnected() {
        ...
        Swal.fire({
            ...
            html: `You are already connected in this room from another active tab/window.`,
            showDenyButton: false,
            confirmButtonText: `OK`,
            showClass: { popup: 'animate__animated animate__fadeInDown' },
            hideClass: { popup: 'animate__animated animate__fadeOutUp' },
        ...
        switch (type) {
            ...
            case mediaType.screen:
                ...
                if (!this.isMobileDevice) {
                    ...
                    this.setTippy(dw.id, 'Toggle draw', 'bottom');
    ...
    exit(offline = false) {
        ...
        const clean = () => {
            ...
            if (this.socket) {
                ...
                this.socket.off('presenterUpdated');
    ...
    async showMessage(data, toggleChat = true) {
        ...
        for (let i = 0; i < participantsListItems.length; i++) {
            ...
            if (li.id === data.peer_id && data.to_peer_id != 'all') {
                ...
                if (this.isToggleUnreadMsg) this.applyUnreadMsgFilter();
            }
        }
    }
    ...
    speechElementText(elemId) {
        ...
        const buttonId = `msg-speech-${elemId.replace('message-', '')}`;
        const isSameActive =
            this.activeSpeechElemId === elemId && (synth.speaking || synth.pending || synth.paused);

        // Toggle off when clicking the same speaker button again.
        if (isSameActive) {
            synth.cancel();
            this.setSpeechButtonState(buttonId, false);
            this.activeSpeechElemId = null;
            this.activeSpeechButtonId = null;
            return;
        }

        // Switch from previous message speech to the current one.
        if (synth.speaking || synth.pending || synth.paused) {
            synth.cancel();
            if (this.activeSpeechButtonId) {
                this.setSpeechButtonState(this.activeSpeechButtonId, false);
            }
        }

        const speech = new SpeechSynthesisUtterance();
        speech.text = element.innerText;
        speech.rate = 0.9;

        this.activeSpeechElemId = elemId;
        this.activeSpeechButtonId = buttonId;

        speech.onstart = () => {
            this.setSpeechButtonState(buttonId, true);
        };
        speech.onend = () => {
            this.setSpeechButtonState(buttonId, false);
            if (this.activeSpeechElemId === elemId) this.activeSpeechElemId = null;
            if (this.activeSpeechButtonId === buttonId) this.activeSpeechButtonId = null;
        };
        speech.onerror = () => {
            this.setSpeechButtonState(buttonId, false);
            if (this.activeSpeechElemId === elemId) this.activeSpeechElemId = null;
    ...
    syncEditorAccessUI() {
        if (typeof quill !== 'undefined' && quill) {
            const canEdit = isPresenter || !this.isEditorLocked;
        ...
        if (isPresenter) {
            ...
            show(btnToShow);
            hide(btnToHide);
            setColor(editorLockBtn, this.isEditorLocked ? 'red' : 'white');
    ...
    handleEditorActionsData(data) {
        ...
        switch (action) {
            ...
            case 'lock':
                this.isEditorLocked = true;
                if (!isPresenter) quill.enable(false);
                this.syncEditorAccessUI();
    ...
    showRecordingInfo(recType, recordingInfo, recordingMsg = '') {
        ...
        if (window.sessionStorage.isReconnected === 'false') {
            Swal.fire({
                background: swalBackground,
                position: 'top',
                title: 'Recording',
    ...
    sendFileInformations(file, peer_id, broadcast = false) {
        ...
        if (this.fileToSend && this.fileToSend.size > 0) {
            ...
            const targetPeerEl = this.getId(peer_id);
            const targetPeerName = broadcast
                ? 'all'
                : targetPeerEl?.getAttribute('data-to-name') || targetPeerEl?.dataset?.toName || peer_id;

            const fileInfo = {
                peer_id: peer_id,
                broadcast: broadcast,
                from_peer_id: this.peer_id,
                to_peer_id: broadcast ? 'all' : peer_id,
                to_peer_name: targetPeerName,
                peer_name: this.peer_name,
                peer_avatar: this.peer_avatar,
    ...
    handleFileInfo(data) {
        ...
        const fromPeerId = this.incomingFileInfo.from_peer_id || this.incomingFileInfo.peer_id;
        const toPeerId = this.incomingFileInfo.to_peer_id || (this.incomingFileInfo.broadcast ? 'all' : this.peer_id);
        const toPeerName = this.incomingFileInfo.to_peer_name || (toPeerId === 'all' ? 'all' : this.peer_name);
    ...
    searchPeer() {
        ...
        const participantsList = this.getId('participantsList');
        const participantsListItems = participantsList.querySelectorAll('li[data-to-name]');
    ...
    applyRaiseHandsFilter() {
        const participantsList = this.getId('participantsList');
        const participantsListItems = participantsList.querySelectorAll('li[data-to-name]');

        for (let i = 0; i < participantsListItems.length; i++) {
            const li = participantsListItems[i];
            const hasRaisedHand = li.querySelector('i.pulsate') !== null;
            const shouldDisplay = !this.isToggleRaiseHand || hasRaisedHand;
            li.style.display = shouldDisplay ? '' : 'none';
        }
    }
    ...
    applyUnreadMsgFilter() {
        const participantsList = this.getId('participantsList');
        const participantsListItems = participantsList.querySelectorAll('li[data-to-name]');

        for (let i = 0; i < participantsListItems.length; i++) {
            const li = participantsListItems[i];
            const shouldDisplay = !this.isToggleUnreadMsg || li.classList.contains('pulsate');
            li.style.display = shouldDisplay ? '' : 'none';
        }
    }
    ...
    syncModeratorUI() {
        if (!isPresenter) return;
        if (!this._moderator) return;

        console.log('Syncing Moderator UI...');
        if (this.getId('switchEveryonePrivacy'))
            this.getId('switchEveryonePrivacy').checked = !!this._moderator.video_start_privacy;
        if (this.getId('switchEveryoneMute'))
            this.getId('switchEveryoneMute').checked = !!this._moderator.audio_start_muted;
        if (this.getId('switchEveryoneHidden'))
            this.getId('switchEveryoneHidden').checked = !!this._moderator.video_start_hidden;
        if (this.getId('switchEveryoneCantUnmute'))
            this.getId('switchEveryoneCantUnmute').checked = !!this._moderator.audio_cant_unmute;
        if (this.getId('switchEveryoneCantUnhide'))
            this.getId('switchEveryoneCantUnhide').checked = !!this._moderator.video_cant_unhide;
        if (this.getId('switchEveryoneCantShareScreen'))
            this.getId('switchEveryoneCantShareScreen').checked = !!this._moderator.screen_cant_share;
        if (this.getId('switchEveryoneCantChatPrivately'))
            this.getId('switchEveryoneCantChatPrivately').checked = !!this._moderator.chat_cant_privately;
        if (this.getId('switchEveryoneCantChatChatGPT'))
            this.getId('switchEveryoneCantChatChatGPT').checked = !!this._moderator.chat_cant_chatgpt;
        if (this.getId('switchEveryoneCantChatDeepSeek'))
            this.getId('switchEveryoneCantChatDeepSeek').checked = !!this._moderator.chat_cant_deep_seek;
        if (this.getId('switchEveryoneCantMediaSharing'))
            this.getId('switchEveryoneCantMediaSharing').checked = !!this._moderator.media_cant_sharing;
        if (this.getId('switchDisconnectAllOnLeave'))
            this.getId('switchDisconnectAllOnLeave').checked = !!this._moderator.disconnect_all_on_leave;
    }
        ...
        if (emit) {
            switch (type) {
                ...
                case 'hand':
                    ...
                    if (this.isToggleRaiseHand) this.applyRaiseHandsFilter();
                    break;
                default:
                    break;
            }
