...
const views = {
    ...
    lobby: path.join(__dirname, '../../', 'public/views/lobby.html'),
    ...
    */
    ...
    app.get('/join/', async (req, res) => {
        if (Object.keys(req.query).length > 0) {
            ...
            if (room && (hostCfg.authenticated || isPeerValid)) {
                // If it's a participant and no presenter is in the room, redirect to lobby
                if (!isPeerPresenter && (!presenters[room] || Object.keys(presenters[room]).length === 0)) {
                    log.debug('Participant redirected to lobby', { room, isPeerPresenter });
                    return htmlInjector.injectHtml(views.lobby, res);
                }
    ...
    app.get('/join/:roomId', async (req, res) => {
        ...
        if (allowRoomAccess) {
            ...
            if (!OIDC.enabled && hostCfg.protected && !hostCfg.users_from_db) {
                ...
                if (roomExists) {
                    if (!presenters[roomId] || Object.keys(presenters[roomId]).length === 0) {
                        log.debug('Participant redirected to lobby (roomId)', { roomId });
                        return htmlInjector.injectHtml(views.lobby, res);
                    }
                    return htmlInjector.injectHtml(views.room, res);
                }
            ...
            // 3. Check for lobby if no protection or already allowed
            if (!presenters[roomId] || Object.keys(presenters[roomId]).length === 0) {
                log.debug('Participant redirected to lobby (roomId fallback)', { roomId });
                return htmlInjector.injectHtml(views.lobby, res);
            }
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('lobbyJoin', ({ roomId }) => {
            log.debug('Lobby join', { roomId, socketId: socket.id });
            socket.room_id = roomId;
            socket.join(roomId);

            // Check how many people are in this room
            const clients = io.sockets.adapter.rooms.get(roomId);
            log.debug('Lobby join - current room size', { roomId, size: clients ? clients.size : 0 });
        });
        ...
        socket.on('join', async (dataObject, cb) => {
            ...
            if (hostCfg.user_auth || peer_token) {
                ...
                const roomAllowedForUser = hostCfg.users_from_db
                    ? await isRoomAllowedForUser('[Join]', peer_name, room.id)
            ...
            // - same peer_uuid with active socket: reject duplicate join
            // - same peer_uuid with stale socket: cleanup and allow reconnect
            // - same peer_name with different uuid: reject as duplicate identity in room
            const roomPeers = room.getPeers();
            let duplicatePeerByUUID = null;
            let duplicatePeerByName = null;

            for (const [existingSocketId, existingRoomPeer] of roomPeers.entries()) {
                if (existingSocketId === socket.id) continue;

                const existingPeerInfo = existingRoomPeer?.peer_info || {};

                if (!duplicatePeerByUUID && peer_uuid && existingPeerInfo.peer_uuid === peer_uuid) {
                    duplicatePeerByUUID = { socketId: existingSocketId, peer: existingRoomPeer };
                }

                if (!duplicatePeerByName && existingPeerInfo.peer_name === peer_name) {
                    duplicatePeerByName = { socketId: existingSocketId, peer: existingRoomPeer };
                }
            }

            if (duplicatePeerByUUID) {
                const { socketId: oldSocketId, peer: oldPeer } = duplicatePeerByUUID;
                const oldSocket = io.sockets.sockets.get(oldSocketId);
                const oldSocketConnected = Boolean(oldSocket && oldSocket.connected);

                if (oldSocketConnected) {
                    log.warn('[Join] - Duplicate active join attempt detected', {
                        room_id: room.id,
                        old_socket_id: oldSocketId,
                        new_socket_id: socket.id,
                        peer_name: oldPeer?.peer_name || peer_name,
                        peer_uuid: oldPeer?.peer_uuid || peer_uuid,
                    });
                    return cb('alreadyJoined');
                }

                log.warn('[Join] - Cleaning stale peer connection with same UUID', {
                    room_id: room.id,
                    old_socket_id: oldSocketId,
                    new_socket_id: socket.id,
                    peer_name: oldPeer?.peer_name || peer_name,
                ...
                if (presenters[socket.room_id] && presenters[socket.room_id][oldSocketId]) {
                    delete presenters[socket.room_id][oldSocketId];
                }

                room.removePeer(oldSocketId);

                if (oldSocket) {
                    oldSocket.leave(socket.room_id);
                    oldSocket.disconnect(true);
                }
            } else if (duplicatePeerByName) {
                log.warn('[Join] - Duplicate peer name already active in room', {
                    room_id: room.id,
                    peer_name,
                    peer_uuid,
                    active_socket_id: duplicatePeerByName.socketId,
                    new_socket_id: socket.id,
                });
                return cb('nameAlreadyInRoom');
            }
            ...
            if (
                ...
                // Notify waiting participants in the lobby
                log.debug('Presenter joined, broadcasting webinarStarted', { room_id: socket.room_id });
                io.to(socket.room_id).emit('webinarStarted', { roomId: socket.room_id });
            }
        ...
        // ####################################################
        // GRANT / REVOKE PRESENTER (ADMIN) ACCESS
        // ####################################################

        socket.on('grantPresenter', (dataObject) => {
            if (!roomExists(socket)) return;
            ...
            // Only an existing presenter can grant or revoke presenter rights
            const callerIsPresenter = isPeerPresenter(socket.room_id, socket.id, peer_name, peer_uuid);
            if (!callerIsPresenter) {
                log.warn('[grantPresenter] Non-presenter attempted to change presenter rights', { peer_name });
                return;
            }

            const { to_peer_id, to_peer_name, to_peer_uuid, grant } = data;

            // Update the in-memory presenters registry
            if (!presenters[socket.room_id]) presenters[socket.room_id] = {};

            if (grant) {
                // Add the target peer as a presenter
                presenters[socket.room_id][to_peer_id] = {
                    peer_name: to_peer_name,
                    peer_uuid: to_peer_uuid,
                    is_presenter: true,
                };
            } else {
                // Do not revoke if peer is in the static allow-list
                if (!hostCfg?.presenters?.list?.includes(to_peer_name)) {
                    delete presenters[socket.room_id][to_peer_id];
                }
            }

            // Reflect the change in the Peer object so live queries are consistent
            const targetPeer = room.getPeer(to_peer_id);
            if (targetPeer) {
                targetPeer.updatePeerInfo({ type: 'presenter', status: !!grant });
            }

            log.debug('[grantPresenter] Presenter rights updated', {
                room_id: socket.room_id,
                to_peer_id,
                to_peer_name,
                grant: !!grant,
                granted_by: peer_name,
            });

            // Tell the target peer their new status directly
            room.sendTo(to_peer_id, 'presenterUpdated', {
                granted: !!grant,
                from_peer_name: peer_name,
                to_peer_id,
                to_peer_name,
            });

            // Keep the newly granted/revoked presenter aligned with current room moderator settings
            // so their Settings -> Moderator tab behaves consistently.
            if (grant) {
                room.sendTo(to_peer_id, 'updateRoomModeratorALL', room.toJson().moderator);
            }

            // Broadcast updated peer_presenter flag to everyone else so the UI refreshes
        ...
        socket.on('fileInfo', (dataObject) => {
            ...
            const isBroadcast = data.broadcast === true || data.broadcast === 'true';
            isBroadcast ? room.broadCast(socket.id, 'fileInfo', data) : room.sendTo(data.peer_id, 'fileInfo', data);
        });
        ...
        socket.on('file', (data) => {
            ...
            const isBroadcast = data.broadcast === true || data.broadcast === 'true';
            isBroadcast ? room.broadCast(socket.id, 'file', data) : room.sendTo(data.peer_id, 'file', data);
        });
        ...
        socket.on('editorActions', (dataObject) => {
            ...
            if (data.action === 'lock') room._isEditorLocked = true;
            if (data.action === 'unlock') room._isEditorLocked = false;
        ...
        socket.on('disconnect', (reason) => {
            ...
            if (presenters[socket.room_id] && presenters[socket.room_id][socket.id]) {
                log.debug('[Disconnect] - Removing presenter from registry', { room_id: socket.room_id, socket_id: socket.id });
        ...
        socket.on('exitRoom', (_, callback) => {
            ...
            if (presenters[socket.room_id] && presenters[socket.room_id][socket.id]) {
                log.debug('[ExitRoom] - Removing presenter from registry', { room_id: socket.room_id, socket_id: socket.id });
                delete presenters[socket.room_id][socket.id];
            }
