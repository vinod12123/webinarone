...
class HtmlInjector {
    ...
    getInjectData() {
        return {
            ...
            OG_SITE_NAME: this.config?.og?.siteName || 'Webinar',
            OG_TITLE: this.config?.og?.title || 'Click the link to make a call.',
            OG_DESCRIPTION:
                this.config?.og?.description ||
                'Webinar calling provides real-time video calls, messaging and screen sharing.',
            OG_IMAGE: this.config?.og?.image || 'https://demo.jobprep.io/logo-jobprep.png',
            OG_URL: this.config?.og?.url || 'https://demo.jobprep.io',
